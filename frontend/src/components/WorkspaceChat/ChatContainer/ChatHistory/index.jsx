...
export default function ChatHistory({ history = [], workspace, sendCommand }) {
    ...
    return (
      <div className="flex flex-col h-full md:mt-0 pb-44 md:pb-40 w-full justify-end items-center">
        <div className="flex flex-col items-center md:items-start md:max-w-[600px] w-full px-4">
          <p className="text-typography-700/80 text-lg font-base py-4">
            Welcome to your new workspace.
          </p>
          {!user || user.role !== "default" ? (
            <p className="w-full items-center text-typography-700/80 text-lg font-base flex flex-col md:flex-row gap-x-1">
  ...
  return (
    <div
      className="markdown text-typography-700 font-light text-sm h-full md:h-[83%] pb-[100px] pt-6 md:pt-0 md:pb-20 md:mx-0 overflow-y-scroll flex flex-col justify-start no-scroll"
      id="chat-history"
      ref={chatHistoryRef}
    >
      ...
      {!isAtBottom && (
        <div className="fixed bottom-40 right-10 md:right-20 z-50 cursor-pointer animate-pulse">
          <div className="flex flex-col items-center">
            <div className="p-1 rounded-full border border-white/10 bg-white/10 hover:bg-white/20 hover:text-typography-700">
              <ArrowDown
                weight="bold"
                className="text-typography-700/60 w-5 h-5"
...
function WorkspaceChatSuggestions({ suggestions = [], sendSuggestion }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-typography-700/60 text-xs mt-10 w-full justify-center">
