...
export default function ChatRow({ chat, onDelete }) {
  ...
  return (
    <>
      <tr className="bg-transparent text-typography-700 text-opacity-80 text-sm font-medium">
        <td className="px-6 py-4 font-medium whitespace-nowrap text-typography-700">
          {chat.id}
        </td>
        <td className="px-6 py-4 font-medium whitespace-nowrap text-typography-700">
          {chat.user?.username}
    ...
    <div className="relative w-full md:max-w-2xl max-h-full">
      <div className="relative bg-main-gradient rounded-lg shadow">
        <div className="flex items-start justify-between p-4 border-b rounded-t border-gray-600">
          <h3 className="text-xl font-semibold text-typography-700">Viewing Text</h3>
          <button
            onClick={closeModal}
            type="button"
        ...
        <div className="w-full p-6">
          <pre className="w-full h-[200px] py-2 px-4 whitespace-pre-line overflow-auto rounded-lg bg-zinc-900 border border-gray-500 text-typography-700 text-sm">
            {text}
          </pre>
        </div>
      </div>
