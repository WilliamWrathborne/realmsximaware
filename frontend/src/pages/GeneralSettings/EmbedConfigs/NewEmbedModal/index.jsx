    ...
    <div className="relative w-full max-w-2xl max-h-full">
      <div className="relative bg-main-gradient rounded-lg shadow">
        <div className="flex items-start justify-between p-4 border-b rounded-t border-gray-500/50">
          <h3 className="text-xl font-semibold text-typography-700">
        ...
        <form onSubmit={handleCreate}>
          <div className="p-6 space-y-6 flex h-auto max-h-[80vh] w-full overflow-y-scroll">
            <div className="w-full flex flex-col gap-y-6">
              ...
              <p className="text-typography-700 text-xs md:text-sm pb-8">
                After creating an embed you will be provided a link that you can
                publish on your website with a simple
                <code className="bg-stone-800 text-typography-700 mx-1 px-1 rounded-sm">
                  &lt;script&gt;
                </code>{" "}
                tag.
              </p>
          ...
          <div className="flex w-full justify-between items-center p-6 space-x-2 border-t rounded-b border-gray-500/50">
            <button
              onClick={closeModal}
              type="button"
              className="px-4 py-2 rounded-lg text-typography-700 hover:bg-stone-900 transition-all duration-300"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="transition-all duration-300 border border-slate-200 px-4 py-2 rounded-lg text-typography-700 text-sm items-center flex gap-x-2 hover:bg-slate-200 hover:text-slate-800 focus:ring-gray-800"
            >
              Create embed
            </button>
          </div>
    ...
    <div>
      <div className="flex flex-col mb-2">
        <label
          ...
          className="block  text-sm font-medium text-typography-700"
        >
          Workspace
        </label>
    ...
    <div>
      ...
      <div className="mt-2 gap-y-3 flex flex-col">
        ...
        >
          ...
          ></div>
          <div className="text-typography-700 text-sm font-medium font-['Plus Jakarta Sans'] leading-tight">
...
export const NumberInput = ({ name, title, hint, defaultValue = 0 }) => {
  return (
    <div>
      <div className="flex flex-col mb-2">
        <label htmlFor={name} className="block text-sm font-medium text-typography-700">
          {title}
        </label>
        <p className="text-typography-700 text-xs">{hint}</p>
      </div>
      <input
        type="number"
        name={name}
